...
// AP Food Consulting - contact endpoint
// Turnstile + honeypot + optional KV rate limit + Resend
// Service area validation by county
//
...
// Notes:
// - Returns JSON only.
// - Avoids leaking operational details in responses.
// - Logs keep reqId correlation.
// - Service area is enforced server-side.

const SERVICE_COUNTIES = new Set([
  "Miami-Dade",
  "Broward",
  "Palm Beach",
  "Collier",
...
export async function onRequestPost(context) {
  ...
  const log = (...args) =>
    console.log(`[contact ${reqId}]`, ...args);
  ...
  try {
    ...
    const CONTACT_SUBJECT_PREFIX =
      env.CONTACT_SUBJECT_PREFIX ||
      "[AP Food Consulting]";

    const RATE_LIMIT_KV =
      env.RATE_LIMIT_KV; // optional KV binding
    ...
    // ----------------------------
    // IP
    // Rate limiting + Turnstile remoteip
    // ----------------------------
    const ip =
    ...
    if (honeypot) {
      log(
        "Honeypot triggered. Skipping send."
      );
    ...
    const htmlBody = [
      ...
      `<p><strong>Restaurant / Business:</strong> ${escapeHtml(
        safeBusiness
      )}</p>`,

      `<p><strong>Service County:</strong> ${escapeHtml(
        county
      )}</p>`,
      ...
      `<p><small>Locale: ${escapeHtml(
        locale ||
          "(not provided)"
      )}</small></p>`,

      `<p><small>Request ID: ${escapeHtml(
        reqId
      )}</small></p>`,
    ].join("");
...
function json(
  ...
  // when request comes from a browser Origin.
  if (
    cors &&
    cors.allowOrigin
  ) {
